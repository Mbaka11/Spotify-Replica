...
class PlaylistService {
  ...
  async addPlaylist (playlist) {
    ...
    return await this.collection.insertOne(playlist);
  }
  ...
  async updatePlaylist (playlist) {
    ...
    await this.savePlaylistThumbnail(playlist);
    return await this.collection.findOneAndReplace({ id: playlist.id }, playlist);
  }
  ...
  async search (substring, exact) {
    const filter = { name: { $regex: `${substring}`, $options: exact ? "" : "i" } };
    const filter2 = { description: { $regex: `${substring}`, $options: exact ? "" : "i" } };
    const playlists = await this.collection.find( { $or: [filter, filter2] } ).toArray();
    return playlists;
  }
