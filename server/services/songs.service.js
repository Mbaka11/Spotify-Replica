...
class SongService {
  ...
  async updateSongLike (id) {
    ...
    return await this.collection.updateOne( {id}, {$set : {liked : !song.liked}} )
  }
  ...
  async search (substring, exact) {
    ...
    const filter2 = { genre: { $regex: `${substring}`, $options: exact ? "" : "i" } };
    const filter3 = { artist: { $regex: `${substring}`, $options: exact ? "" : "i" } };
